...
static char rcsid[] = "$Id: init.c,v 1.1.1.1 2002/07/08 04:28:54 daa1 Exp $";
...
static int initchar(int len, Type ty) {
	...
	do {
	  (*IR->defconst)(U, 1, initvalue(ty)->u.v);
	  n++;
	  /*
...
static int initfields(Field p, Field q) {
	...
	do {
		...
		if (fieldsize(p) < IR->unit_size*p->type->size) {
			if (p->type == inttype &&
	...
	n = (n + (IR->unit_size - 1))/IR->unit_size;
	for (i = 0; i < n; i++) {
		Value v;
		if (IR->little_endian) {
			...
			v.u = (unsigned char)(bits>>(IR->unit_size*(unsignedtype->size - 1)));
			bits <<= IR->unit_size;
		}
...
Type initializer(Type ty, int lev) {
	...
	if (isarray(ty) && ischar(aty)) {
		if (t == SCON) {
			...
			n = tsym->type->size;
		  fprintf(stdout, "calling defstring from initializer\n");
