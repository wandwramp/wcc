...
static char rcsid[] = "$Id: stab.c,v 1.1.1.1 2002/07/08 04:28:54 daa1 Exp $";
...
/*              fprint(2,"`%t'=%d\n", ty, tc); */
	...
	case STRUCT: case UNION: {
		...
		for (p = fieldlist(ty); p; p = p->link) {
			...
			if (p->lsb)
				print(",%d,%d;", IR->unit_size*p->offset +
					(IR->little_endian ? fieldright(p) : fieldleft(p)),
					fieldsize(p));
			else
				print(",%d,%d;", IR->unit_size*p->offset, IR->unit_size*p->type->size);
