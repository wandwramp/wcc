...
#define extend(x,ty) ((x)&(1<<(IR->unit_size*(ty)->size-1)) ? (x)|((~0UL)<<(IR->unit_size*(ty)->size-1)) : (x)&ones(IR->unit_size*(ty)->size))
...
#define fieldleft(p)  (IR->unit_size*(p)->type->size - \
...
typedef struct interface {
  ...
  /* Added 3/1/2002 by Dean Armstrong */
