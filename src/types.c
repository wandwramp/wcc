...
static char rcsid[] = "$Id: types.c,v 1.1.1.1 2002/07/08 04:28:54 daa1 Exp $";
...
static Type xxinit(int op, char *name, Metrics m) {
	...
	switch (ty->op) {
	case INT:
		p->u.limits.max.i = ones(IR->unit_size*ty->size)>>1;
		p->u.limits.min.i = -p->u.limits.max.i - 1;
		break;
	case UNSIGNED:
		p->u.limits.max.u = ones(IR->unit_size*ty->size);
		p->u.limits.min.u = 0;
		break;
	case FLOAT:
...
#undef xx
	...
	pointersym->u.limits.max.p = (void*)ones(IR->unit_size*IR->ptrmetric.size);
