...
static char rcsid[] = "$Id: decl.c,v 1.1.1.1 2002/07/08 04:28:54 daa1 Exp $";

#define add(x,n) (x > inttype->u.sym->u.limits.max.i-(n) ? (overflow=1,x) : x+(n))
#define chkoverflow(x,n) ((void)add(x,n))
#define bits2bytes(n) (((n) + (IR->unit_size - 1))/IR->unit_size)
static int regcount;
...
static void fields(Type ty) {
	  ...
	  while (istypename(t, tsym)) {
	  	...
	  	for (;;) {
			...
			if (t == ':') {
				...
				if (p->bitsize > IR->unit_size*inttype->size || p->bitsize < 0) {
					error("`%d' is an illegal bit-field size\n",
						p->bitsize);
					p->bitsize = IR->unit_size*inttype->size;
	...
	{ int bits = 0, off = 0, overflow = 0;
	  ...
	  for (p = *q; p; p = p->link) {
		...
		|| bits - 1 + p->bitsize > IR->unit_size*unsignedtype->size) {
		...
		if (p->lsb) {
			...
			if (IR->little_endian)
				...
				p->lsb = IR->unit_size*unsignedtype->size - bits + 1
					- p->bitsize + 1;
			bits += p->bitsize;
		} else
