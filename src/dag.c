...
static char rcsid[] = "$Id: dag.c,v 1.1.1.1 2002/07/08 04:28:54 daa1 Exp $";
...
Node listnodes(Tree tp, int tlab, int flab) {
	...
	case ASGN:  { assert(tlab == 0 && flab == 0);
			...
			if (fieldsize(f) < IR->unit_size*f->type->size) {
	...
	case FIELD: { Tree q = tp->kids[0];
		      ...
		      } else if (fieldsize(tp->u.field) < IR->unit_size*tp->u.field->type->size)
		      	q = bittree(BAND,
